...
def plot_stacked_bars__(
    df: pd.DataFrame,
    title: str,
    out_path: Path,
    model_np: int = 1,
    zoom_upper: float | None = None,     # upper limit (minutes) for the zoomed bottom panel
    tiny_label_threshold: float = 0.5,   # annotate slices smaller than this (minutes)
):
    # Prep & equal spacing ------------------------------------------------------
    bars = (
    ...
    ranks = bars["ranks"].to_numpy()
    ranks_total = ranks * (model_np + 1)      # show as tick labels
    x = np.arange(len(ranks_total))           # ensures equal spacing
    width = 0.65

    col_order = ["analysis_m", "forecast_m", "ens_init_m", "io_m"]
    label_map = {"analysis_m": "Analysis", "forecast_m": "Forecast",
                 "ens_init_m": "Ens Init", "io_m": "Io"}

    # Choose a sensible zoom range ---------------------------------------------
    totals = bars[col_order].sum(axis=1).to_numpy()
    max_total = float(np.max(totals)) if len(totals) else 1.0

    if zoom_upper is None:
        # Detect "dominant" category (usually Forecast) and zoom to fit the rest.
        sums_by_col = {c: float(bars[c].sum()) for c in col_order}
        dominant = max(sums_by_col, key=sums_by_col.get)
        small_cols = [c for c in col_order if c != dominant]
        max_small_slice = float(np.max([bars[c].to_numpy().max() for c in small_cols])) if small_cols else 0.0

        # Heuristics: show all the small slices comfortably, but keep zoom panel
        # clearly smaller than the full range.
        zoom_upper = max(1e-3, min(max_total * 0.25, max_small_slice * 1.25))  # clamp to <= 25% of full

    # Figure layout with broken y-axis ------------------------------------------
    fig = plt.figure(figsize=(11.5, 7), dpi=160)
    gs = gridspec.GridSpec(nrows=2, ncols=1, height_ratios=[3.0, 1.4], hspace=0.05)
    ...
    def draw_stack(ax):
        ...
        for col in col_order:
            ...
            ax.bar(x, vals, width=width, bottom=bottom,
                   label=label_map[col], edgecolor="black", linewidth=0.6, alpha=0.9)
            # annotate very small slices
            for xi, v, b in zip(x, vals, bottom):
                if 0 < v < tiny_label_threshold:
                    ax.text(xi, b + v * 1.05, f"{v:.2f}m",
                            ha="center", va="bottom", fontsize=9)
            bottom += vals
        return bottom

    totals_drawn = draw_stack(ax_top)
    draw_stack(ax_bot)

    # Y limits: bottom shows [0, zoom_upper], top shows [gap, max]
    ax_bot.set_ylim(0, max(zoom_upper, 1e-6))
    ax_top.set_ylim(max(zoom_upper * 1.05, np.min(totals_drawn) * 0.95), max_total * 1.08)

    # Diagonal break marks ------------------------------------------------------
    def break_marks(upper_ax, lower_ax):
        d = 0.015
        kwargs = dict(transform=upper_ax.transAxes, color='k', clip_on=False, linewidth=1.2)
        upper_ax.plot((-d, +d), (-d, +d), **kwargs)                # left
        upper_ax.plot((1 - d, 1 + d), (-d, +d), **kwargs)          # right
        kwargs = dict(transform=lower_ax.transAxes, color='k', clip_on=False, linewidth=1.2)
        lower_ax.plot((-d, +d), (1 - d, 1 + d), **kwargs)          # left
        lower_ax.plot((1 - d, 1 + d), (1 - d, 1 + d), **kwargs)    # right

    break_marks(ax_top, ax_bot)
    ...
    ax_top.set_title(title, fontdict={"fontsize": 14, "fontweight": "bold"})
    ax_top.legend(loc="upper right", prop={"size": 13, "weight": "bold"})

    ax_bot.set_xlabel("Ranks (MPI processes)", fontdict={"fontsize": 16, "fontweight": "bold"})
    ax_top.set_ylabel("Time (minutes)",         fontdict={"fontsize": 16, "fontweight": "bold"})
    ax_bot.set_ylabel("Time (minutes)",         fontdict={"fontsize": 16, "fontweight": "bold"})

    # Equal spacing: ticks at indices, labels are true total ranks
    ax_bot.set_xticks(x)
    ax_bot.set_xticklabels(ranks_total, fontsize=14, fontweight="bold")
    plt.setp(ax_top.get_xticklabels(), visible=False)  # hide top x labels
...
def plot_stacked_bars_(df: pd.DataFrame, title: str, out_path: Path, model_np=1):
    ...
    for col in ["analysis_m", "forecast_m", "ens_init_m", "io_m"]:
        ...
        ax.bar(
            x, vals, width=width, bottom=bottom, label=nice[col],
            edgecolor="black", linewidth=0.6, alpha=0.9
        )
        bottom += vals

    # Axis cosmetics
    ax.set_xlabel("Ranks (MPI processes)", fontdict={"fontsize": 16, "fontweight": "bold"})
    ax.set_ylabel("Time (minutes)",        fontdict={"fontsize": 16, "fontweight": "bold"})
    ax.set_title(title,                    fontdict={"fontsize": 16, "fontweight": "bold"})
    ax.grid(True, axis="y", linestyle="--", alpha=0.6)

    # Equal spacing: ticks at the evenly spaced indices, labels are the true ranks
    ax.set_xticks(x)
    ax.set_xticklabels(ranks_total, fontsize=16, fontweight="bold")
    ax.tick_params(axis="y", labelsize=16)
    ax.set_yticklabels([f"{N:.0f}" for N in ax.get_yticks()], fontsize=16, fontweight="bold")
