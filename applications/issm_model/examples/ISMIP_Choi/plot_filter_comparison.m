...
run_def = struct( ...
    'key',    {'WBF','EBF','IBF'}, ...
    'folder', {'_modelrun_datasets_run_wbf', ...
               '_modelrun_datasets_run_ebf', ...
               '_modelrun_datasets_run_ibf'}, ...
...
assimilation_times = 2:1:36;      % annual state observations; [] hides guides
...
% Basal friction is meaningful only beneath grounded ice. Mask the truth
% with the true GL and each recovered field with that experiment's own GL.
% Using the final true-grounded mask for every panel imprints the answer's
% boundary on WBF/EBF/IBF and can make a misplaced GL look artificially exact.
...
map_data{1}(~true_grounded_final) = NaN;
for p = 2:numel(map_data)
    run_index = p - 1;
    estimate_final = runs(run_index).estimate(:,runs(run_index).final_k);
    estimate_H = estimate_final(1:n_nodes);
    estimate_bed = estimate_final(4*n_nodes+1:5*n_nodes);
    estimate_grounded = estimate_H > 0 & ...
        estimate_H + estimate_bed/density_ratio > 0;
    map_data{p}(~estimate_grounded) = NaN;
end
map_titles = { ...
